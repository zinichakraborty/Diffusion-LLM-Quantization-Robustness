...
1. Install `uv`: <https://docs.astral.sh/uv/getting-started/installation/>. On PACE, do `module load uv` 
2. Set the following environment variables:
```bash
export SLURM_INCLUDE_DIR=/opt/slurm/current/include
export SLURM_LIB_DIR=/opt/slurm/current/lib
```
3. Run `uv sync --no-build-isolation` to setup virtual environment
