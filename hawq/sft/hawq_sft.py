...
def load_coda_or_auto(model_name, model_kwargs):
    ...
    - If model_name is a directory with config.json -> treat as local checkpoint.
    - Otherwise -> assume it's a HF repo ID, try local CoDA implementation first.
    """
    # Case 1: local checkpoint directory
    # if os.path.isdir(model_name) and os.path.exists(os.path.join(model_name, "config.json")):
    #     print(f"Detected local checkpoint at: {model_name}")
    #     print("Loading with AutoModel (trust_remote_code=True)")
    #     return AutoModel.from_pretrained(
    #         model_name,
    #         **model_kwargs,
    #     )

    # Case 2: HF repo ID (e.g., 'Salesforce/CoDA-v0-Instruct')
...
# ============================================================================
# Training Loop for CoDA (GSM8K SFT or WikiText-2 pretrain)
# ============================================================================
...
def main():
    parser = argparse.ArgumentParser(
        description="CoDA DDM Training (GSM8K SFT or WikiText-2 pretraining)",
